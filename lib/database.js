 ...
 */
async function get_user_token(pub_key) {
  debug(`Checking for user with pub_key ${pub_key}`);
  const q = `SELECT token, sandbox
               from push_tokens
               WHERE pub_key = $1`;
  const res = await pool.query(q, [pub_key.toLowerCase()]);
  return res.rows;
}
...
async function register_token({
  ...
  return pool
    .query(q, [pub_key.toLowerCase(), token, platform, sandbox, appId, userId])
    .then((res) => {
...
async function schedule_reminder(pub_key, reminder_type, send_at) {
  ...
  return pool
    .query(q, [pub_key.toLowerCase(), reminder_type, send_at])
    .then((res) => {
      const success = res.rowCount > 0;
      if (!success) {
