...
async function schedule_reminders_for_recipient(recipient_pub_key, sender_pub_key) {
    try {
        await db.schedule_reminder(
            recipient_pub_key,
            'recipient_1',
            new Date().addHours(REMINDER_TYPES.recipient_1.hours_from_now)
        );
        await db.schedule_reminder(
            recipient_pub_key,
            'recipient_2',
            new Date().addHours(REMINDER_TYPES.recipient_2.hours_from_now)
        );
        await db.schedule_reminder(
            recipient_pub_key,
            'recipient_expired',
            new Date().addHours(REMINDER_TYPES.recipient_expired.hours_from_now)
        );
        await db.schedule_reminder(
            sender_pub_key,
            'sender_expired',
            new Date().addHours(REMINDER_TYPES.sender_expired.hours_from_now)
        );
    } catch (error) {
...
async function schedule_reminders_for_sender(recipient_pub_key, sender_pub_key) {
    try {
        await db.schedule_reminder(
            sender_pub_key,
            'sender_1',
            new Date().addHours(REMINDER_TYPES.sender_1.hours_from_now)
        );
        await db.schedule_reminder(
            sender_pub_key,
            'sender_2',
            new Date().addHours(REMINDER_TYPES.sender_2.hours_from_now)
        );
        await db.schedule_reminder(
            sender_pub_key,
            'sender_broadcast_expired',
            new Date().addHours(REMINDER_TYPES.sender_broadcast_expired.hours_from_now)
        );
        await db.schedule_reminder(
            recipient_pub_key,
            'recipient_broadcast_expired',
            new Date().addHours(REMINDER_TYPES.recipient_broadcast_expired.hours_from_now)
        );
    } catch (error) {
        console.error('Error scheduling a reminder for recipient');
        console.error(error);
...
async function process_reminders() {
    ...
    for (let index = 0; index < rows.length; index++) {
        ...
        let sandbox = false;
        try {
            const tokenRow = await db.get_user_token({ pubKey: pub_key });
            if (!tokenRow) {
