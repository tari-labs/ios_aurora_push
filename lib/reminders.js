...
async function process_reminders() {
    ...
    for (let index = 0; index < rows.length; index++) {
        const { id, pub_key, reminder_type, scheduled_for } = rows[index];

        //Make sure the reminder isn't stale
        const hoursPastScheduledTime = (new Date() - new Date(scheduled_for).getTime()) / 1000 / 60 / 60;
        ...
        const payload = {
            ...
            pushType: "alert",
            sound: 'ping.aiff',
            expiry: Math.floor(Date.now() / 1000) + (60 * 60 * process.env.EXPIRE_PUSH_AFTER_HOURS || 2)
        };
